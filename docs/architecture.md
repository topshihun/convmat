...
> 注意：runmat 目前是 pre-1.0，MIR 语义仍在演化。降级器应集中在一处（见 §8
...
- 形状来源：静态分析只有字面量 + 形状传播（转置/乘/按维归约）；形参默认标量，
  数组形参需 `runmat-static-analysis`（见 §10.5）。
...
> 受 `runmat-static-analysis` 依赖阻碍（见 §10.5）。
...
- 与静态形状（§11.2）不同，这里的「未知」不止是维度，还包括「有多少个参数、每个
  参数是什么类型」，是更高一层的动态。
