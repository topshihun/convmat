...
pub enum Builtin {
    ...
    /// over one array.
    MinMax(MinMax),
    /// A reduction over an array to a scalar (`sum`, `prod`).
    Reduce(ReduceOp),
}
...
impl Builtin {
    ...
    pub fn valid_arity(self, n: usize) -> bool {
        match self {
            Builtin::Unary(_) | Builtin::Sign | Builtin::Reduce(_) => n == 1,
            Builtin::Binary(_) => n == 2,
            Builtin::MinMax(_) => n == 1 || n == 2,
        }
    }
}
...
pub fn libm_symbol(builtin: Builtin) -> Option<&'static str> {
    match builtin {
        ...
        Builtin::Sign | Builtin::Reduce(_) => None,
